...
export default {
  ...
  computed: {
    ...mapState(['tutors']),
    ...mapGetters(['isAuthenticated', 'loggedInUser', 'locations']),
    ...mapGetters({
  ...
  async mounted() {
    await this.$axios
      .get(this.$store.state.baseUrl + '/user_membership', {})
      .then((result) => {
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (this.membership === 0) {
        this.formIsValid = false
        this.$toast.error('You must upgrade your memebership', {
          position: 'top-right',
          duration: 3000,
        })
        return
      ...
      const message = {
        ...
        messagee_id: this.selectedTutor.id,
      }
    ...
    initiateMap() {
      const lat = this.selectedTutor.postcode.lat
      const long = this.selectedTutor.postcode.long
